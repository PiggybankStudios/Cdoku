	...
	if (BtnPressed(Btn_A))
	{
		...
		if (mmenu->selectionIndex >= 0 && (u64)mmenu->selectionIndex < mmenu->buttons.length)
		{
			...
			switch (selectedBtn->action)
			{
				...
				{
					#if 0
					MainMenuGotoSubMenu(MMenuSubMenu_Settings);
					#elif DEBUG_BUILD
					u64 dummyAllocSize = (MAIN_HEAP_PAGE_SIZE / 4) - sizeof(HeapAllocPrefix_t) - 2;
					void* dummyAllocation = AllocMem(mainHeap, dummyAllocSize);
					PrintLine_D("Allocating %llu bytes (%p)", dummyAllocSize, dummyAllocation);
					if (dummyAllocation != nullptr)
					{
						MyMemSet(dummyAllocation, 0xDD, dummyAllocSize);
					}
					#endif
				} break;
